...
const getNdmiTileUrlMock = jest.fn<Promise<string>, [unknown, unknown]>();
jest.mock('@/lib/agro/earth-engine', () => ({
    isGeeConfigured: () => isGeeConfiguredMock(),
    getNdmiTileUrl: (aoi: unknown, win: unknown) => getNdmiTileUrlMock(aoi, win),
}));
...
beforeEach(() => {
    ...
    getNdmiTileUrlMock.mockResolvedValue('https://earthengine.googleapis.com/v1/projects/p/maps/m/tiles/{z}/{x}/{y}');
});
...
it('generates + caches a tile URL on a cache miss', async () => {
    ...
    expect(getNdmiTileUrlMock).toHaveBeenCalledWith(
        { west: 10, south: 40, east: 11, north: 41 },
        { start: '2026-05-16', end: '2026-06-15' },
    );
    expect(redisSet).toHaveBeenCalledWith(
        'ndmi:tile:tenant-1:loc-1:clip2:2026-06-15',
        body.tileUrl,
        'EX',
        21_600,
    );
