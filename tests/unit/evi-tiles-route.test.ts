/**
 * Unit test — GET /api/t/[tenantSlug]/agro/evi-tiles.
 *
 * Mirrors the NDVI route test: proves the wiring around the Google
 * Earth Engine EVI (enhanced vegetation index) service via the shared
...
import { GET } from '@/app/api/t/[tenantSlug]/agro/evi-tiles/route';

function call(qs: string) {
    const req = new NextRequest(`http://localhost/api/t/acme/agro/evi-tiles?${qs}`);
...
it('generates + caches a tile URL on a cache miss', async () => {
    ...
    expect(getEviTileUrlMock).toHaveBeenCalledWith(
        { west: 10, south: 40, east: 11, north: 41 },
        { start: '2026-05-16', end: '2026-06-15' },
    );
    expect(redisSet).toHaveBeenCalledWith(
        'evi:tile:tenant-1:loc-1:2026-06-15',
        body.tileUrl,
        'EX',
        21_600,
    );
...
it('degrades softly when GEE generation throws', async () => {
    getEviTileUrlMock.mockRejectedValue(new Error('EE getMap failed'));
    const res = await call('locationId=loc-1&date=2026-06-15');
