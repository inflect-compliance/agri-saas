/**
 * Unit test — GET /api/t/[tenantSlug]/agro/ndwi-tiles.
 *
 * Mirrors the NDVI route test: proves the wiring around the Google Earth
 * Engine NDWI (McFeeters water-index) service:
...
const getNdwiTileUrlMock = jest.fn<Promise<string>, [unknown, unknown]>();
jest.mock('@/lib/agro/earth-engine', () => ({
    isGeeConfigured: () => isGeeConfiguredMock(),
    getNdwiTileUrl: (aoi: unknown, win: unknown) => getNdwiTileUrlMock(aoi, win),
}));
...
import { GET } from '@/app/api/t/[tenantSlug]/agro/ndwi-tiles/route';

function call(qs: string) {
    const req = new NextRequest(`http://localhost/api/t/acme/agro/ndwi-tiles?${qs}`);
...
beforeEach(() => {
    ...
    getNdwiTileUrlMock.mockResolvedValue('https://earthengine.googleapis.com/v1/projects/p/maps/m/tiles/{z}/{x}/{y}');
});
...
it('reports not-configured without calling GEE when creds are absent', async () => {
    ...
    expect(getNdwiTileUrlMock).not.toHaveBeenCalled();
});
...
it('generates + caches a tile URL on a cache miss', async () => {
    ...
    expect(getNdwiTileUrlMock).toHaveBeenCalledWith(
        { west: 10, south: 40, east: 11, north: 41 },
        { start: '2026-05-16', end: '2026-06-15' },
    );
    expect(redisSet).toHaveBeenCalledWith(
        'ndwi:tile:tenant-1:loc-1:2026-06-15',
        body.tileUrl,
        'EX',
        21_600,
    );
...
it('degrades softly when GEE generation throws', async () => {
    getNdwiTileUrlMock.mockRejectedValue(new Error('EE getMap failed'));
    const res = await call('locationId=loc-1&date=2026-06-15');
...
it('still works (uncached) when Redis is unavailable', async () => {
    ...
    const body = await res.json();
    expect(body.tileUrl).toContain('earthengine.googleapis.com');
    expect(getNdwiTileUrlMock).toHaveBeenCalledTimes(1);
});
