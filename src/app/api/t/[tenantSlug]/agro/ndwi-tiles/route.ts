...
import { isGeeConfigured, getNdwiTileUrl } from '@/lib/agro/earth-engine';
import { getRedis } from '@/lib/redis';
import { withApiErrorHandling } from '@/lib/errors/api';
import { jsonResponse } from '@/lib/api-response';

/**
 * NDWI tile source (Agro-intel) — tenant + location scoped, authenticated.
 *
 * Generates a recent cloud-masked Sentinel-2 NDWI (McFeeters water-index)
 * composite for the location's field area via Google Earth Engine and returns
 * the ephemeral XYZ tile-URL the map's raster overlay consumes. EE tile URLs
 * are short-lived, so the result is cached in Redis for `CACHE_TTL_SECONDS`,
...
export const GET = withApiErrorHandling(
    async (req: NextRequest, { params: paramsPromise }: { params: Promise<{ tenantSlug: string }> }) => {
        ...
        const cacheKey = `ndwi:tile:${ctx.tenantId}:${query.locationId}:${end}`;
